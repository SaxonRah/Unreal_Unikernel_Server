...
void BitWriter::write_u8(uint8_t v) { write_bits_u64(v, 8); }

void BitWriter::write_u16(uint16_t v) { write_bits_u64(v, 16); }
...
void BitWriter::write_random_bytes(size_t n) {
  for (size_t i = 0; i < n; ++i) {
    // Same broad shape as UE's non-crypto random filler. Not
    // security-sensitive.
    write_u8((uint8_t)(rand() % 255));
  }
}

void BitWriter::write_termination_bit() { write_bit(true); }
...
void BitReader::set_at_end() { pos_bits_ = total_bits_; }
